...
void InitILI9340()
{
  ...
  BEGIN_SPI_COMMUNICATION();
  {
    ...
    // The following appear in ILI9340 Data Sheet v1.11 (2011/06/10), but not in older v1.02 (2010/12/06).
    ...
    SPI_TRANSFER(0xCF/*Power Control B*/, 0x00/*Always Zero*/, 0xC1/*Power Control=0,DRV_ena=0,PCEQ=1*/, 0x30/*DC_ena=1*/); // Not sure what the effect is, set to default as per ILI9340 Application Notes v0.6 (2011/03/11) document (which is not apparently same as default at power on).
    SPI_TRANSFER(0xE8/*Driver Timing Control A*/, 0x85, 0x00, 0x78); // Not sure what the effect is, set to default as per ILI9340 Application Notes v0.6 (2011/03/11) document (which is not apparently same as default at power on).
    SPI_TRANSFER(0xEA/*Driver Timing Control B*/, 0x00, 0x00); // Not sure what the effect is, set to default as per ILI9340 Application Notes v0.6 (2011/03/11) document (which is not apparently same as default at power on).
    SPI_TRANSFER(0xED/*Power On Sequence Control*/, 0x64, 0x03, 0x12, 0x81); // Not sure what the effect is, set to default as per ILI9340 Application Notes v0.6 (2011/03/11) document (which is not apparently same as default at power on).
#if ILI9340_UPDATE_FRAMERATE == ILI9340_FRAMERATE_119_HZ // Setting pump ratio if update rate is less than 119 Hz does not look good but produces shimmering in panning motion.
    SPI_TRANSFER(0xF7/*Pump Ratio Control*/, 0x30/*DDVDH=3xVCI*/); // Appears in ILI9340 Data Sheet v1.11 (2011/06/10), but not in older v1.02 (2010/12/06). This has a subtle effect on colors/saturation. Valid values are 0x20 and 0x30. Spec says 0x20 is default at boot, but doesn't seem so, more like 0x00 is default, giving supersaturated colors. I like 0x30 best.
#endif
    // The following appear also in old ILI9340 Data Sheet v1.02 (2010/12/06).
...
#define MADCTL_ROTATE_180_DEGREES (MADCTL_COLUMN_ADDRESS_ORDER_SWAP | MADCTL_ROW_ADDRESS_ORDER_SWAP)
    ...
    SPI_TRANSFER(0xB1/*Frame Rate Control (In Normal Mode/Full Colors)*/, 0x00/*DIVA=fosc*/, ILI9340_UPDATE_FRAMERATE/*RTNA(Frame Rate)*/);
//    SPI_TRANSFER(0xB5/*Blanking Porch Control*/, 0x02/*VFP, vertical front porch*/, 0x02/*VBP, vertical back porch*/, 0x0A/*HFP, horizontal front porch*/, 0x14/*HBP, horizontal back porch*/); // These are the default values at power on
    ...
    SPI_TRANSFER(0xF2/*Enable 3G*/, 0x02/*False*/); // This one is present only in ILI9340 Data Sheet v1.11 (2011/06/10)
